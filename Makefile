...
deploy-local: all
	helm upgrade --install --recreate-pods build-mattermost-com --set jenkins.Master.Image=local/jenkins-master --set jenkins.Master.ImagePullPolicy=Never ./mattermost-jenkins/mattermost-jenkins-*.tgz
...
redeploy-local: all
	helm delete --purge build-mattermost-com || echo notfound
	cd mattermost-jenkins && helm install --set jenkins.Master.Image=local/jenkins-master --set jenkins.Master.ImagePullPolicy=Never mattermost-jenkins-*.tgz -n build-mattermost-com
...
destroy:         ## Delete all VMs and run clean
destroy: clean
	minikube delete
